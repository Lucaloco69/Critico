...
export default function Requests() {
  ...
  const loadRequests = async (ownerUserId: number) => {
    try {
      setLoading(true);
      ...
      const myRequests: Request[] = (data ?? [])
        .filter((r) => r.Product?.owner_id === ownerUserId && r.Sender && r.Product)
        .map((r) => ({
          id: r.id,
  ...
  createEffect(() => {
    ...
    const channel = supabase
      .channel("requests_updates_" + uid)
      .on("postgres_changes", { event: "*", schema: "public", table: "Requests" }, () => loadRequests(uid))
      .subscribe();
  ...
  const acceptRequest = async (requestId: number) => {
    try {
      ...
      await grantCommentPermission(req.sender_id, req.product_id);
  ...
  const pendingRequests = createMemo(() => requests().filter((r) => r.status === null));
  const answeredRequests = createMemo(() => requests().filter((r) => r.status !== null));
                  ...
                  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 hover:shadow-lg transition-shadow">
                    <div class="flex items-start gap-4">
                      <div class="w-24 h-24 rounded-lg overflow-hidden bg-gray-200 dark:bg-gray-700 flex-shrink-0">
                        ...
                        >
                          <img src={request.Product.picture!} alt={request.Product.name} class="w-full h-full object-cover" />
                        </Show>
                      </div>

                      <div class="flex-1 min-w-0">
                        <h3 class="font-semibold text-gray-900 dark:text-white text-lg mb-1">{request.Product.name}</h3>
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400">{formatDate(request.created_at)}</p>
                      </div>
                ...
                {(request) => (
                  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 opacity-75">
                    <div class="flex items-start gap-4">
                      ...
                      <div class="flex-1">
                        <h3 class="font-semibold text-gray-900 dark:text-white mb-1">{request.Product.name}</h3>
